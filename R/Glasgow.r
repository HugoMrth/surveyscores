Glasgow <- function(x, inv.scale = FALSE) {
  ...
  if(length(table(x[,1])) > 4) stop("L'echelle d'ouverture des yeux est en plus de 4 niveaux")
  if(length(table(x[,2])) > 5) stop("L'echelle de reponse verbale est en plus de 5 niveaux")
  if(length(table(x[,3])) > 6) stop("L'echelle de reponse motrice est en plus de 6 niveaux")
